...
| El scrape apunta a `host.docker.internal:8080`, `:8085` y `:8089` (Spring) y a `:8185`, `:8189` (Quarkus, en su propio trabajo porque la ruta es `/q/metrics`) | Los servicios corren en el devcontainer, no como contenedores de `aggora-net`. Se probo el nombre del contenedor (`eager_allen`) y **no resuelve** desde Prometheus; `host.docker.internal` si (Docker Desktop lo da hecho en Windows/WSL2; en Linux hay que anadir `extra_hosts: host-gateway`). **Y una trampa que costo un rato**: `host.docker.internal` solo responde para los puertos que el host de WSL2 tiene reenviados, asi que un puerto sin reenviar da `connection refused` y **parece que el servicio esta caido cuando esta perfectamente vivo**. Por eso los puertos de los dos gateways estan declarados en `forwardPorts` del devcontainer en vez de depender del auto-descubrimiento de VS Code (`docs/decisions.md`, seccion del gateway) |
