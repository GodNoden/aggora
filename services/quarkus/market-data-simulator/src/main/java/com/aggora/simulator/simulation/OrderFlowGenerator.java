...
/**
 * Genera ordenes de "traders" simulados hacia {@code orders.incoming}.
 *
 * <p>Vive en el simulador porque este servicio ya es el generador de trafico del proyecto y ya
 * tiene los precios de referencia, que es lo que hace falta para poner precios limite creibles.
 *
 * <p>El precio se pone alrededor de la referencia (+-0,5%) y los lados son aleatorios, asi que
 * unas ordenes cruzan con las que ya estaban en el libro y otras se quedan esperando. Esa mezcla
 * es justo lo que hace que el motor de matching tenga trabajo.
 */
@ApplicationScoped
public class OrderFlowGenerator {
    ...
    private final MutinyEmitter<Order> ordersEmitter;
    private final Random random = new Random();
    private final AtomicLong sent = new AtomicLong();

    @Inject
    public OrderFlowGenerator(AggoraConfig props, TickEngine engine,
                              @Channel("orders-incoming") MutinyEmitter<Order> ordersEmitter) {
        this.props = props;
        this.engine = engine;
        this.ordersEmitter = ordersEmitter;
    }

    /** {@code delayed} es el {@code initialDelay} de Spring: los primeros 15 s no hay ordenes. */
    @Scheduled(delayed = "${aggora.order-initial-delay-ms:15000}ms",
            every = "${aggora.order-interval-ms:1000}ms",
    ...
    public void emitOrders() {
        ...
        AggoraConfig.Instrument instrument = open.get(random.nextInt(open.size()));
        ...
        // instrumento, y para que un libro este completo sus ordenes tienen que caer todas en la
        // misma particion. Con la cuenta como key habria que reparticionar.
        ordersEmitter.sendMessage(KafkaRecord.of(instrument.symbol(), order))
                .subscribe().with(
                        ignorado -> {
                        ...
                        ex -> log.error("[ordenes] fallo enviando {}: {}", instrument.symbol(), ex.getMessage()));
    }
}
